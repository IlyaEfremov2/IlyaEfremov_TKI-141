...
int main(void)
{
	...
	puts("Введите меньшее значение массива:");
	int low_value = get_input();
	puts("Введите большее значение массива:");
	int high_value = get_input();
...
void check_third_element(const size_t size_array)
{
	if (size_array < 2)
	{
		printf("В масиве нет третьего символа, так как его размер меьше 3-х");
		exit(EXIT_FAILURE);
	}
}